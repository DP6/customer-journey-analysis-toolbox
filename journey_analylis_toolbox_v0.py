# -*- coding: utf-8 -*-
"""Caixa de Ferramentas Para Análises de Jornadas """

import pandas as pd
jor = 'A > B > B > C > D > E > E C > D > E > E'

#Funções para o escopo de Jornada. 
def get_size(j,separator = ' > '):
  """
...
def get_distinct_tps(j,separator = ' > '):
  """
  Returns the distict touch points of the journey. 
  ...
  Returns: list. A list of the distinct touch points found in the journey
  given in j.
  """
  distinct_tps = set(j.split(separator))
  result = list(distinct_tps)
  return result

def get_first_tp(j,separator = ' > '):
  """
...
def get_intermediate_tp(j,range, separator=' > '):
  """
...
def get_tps_counts(j,norm=False, separator = ' > '):
  """
...
def skip_tp(j, tp_to_skip, separator = ' > '):
  """
...
def skip_tp_group(j,tps_to_skip, separator = ' > '):  
  """
...
def check_tp(j,tp_to_check,separator = ' > '):
  """
...
def check_tp_group(j, tp_group_to_check, separator = ' > '):
  """
...
def get_tp_counts(j,tp, norm=False, separator = ' > '):
  """
  ...
  return result
get_tp_counts(jor,'B', norm=True)

def get_duration(timestamps,range=(0,-1)):
  """
...
def translate_tp(j,translation_dict,separator = ' > '):  
  """
...
def get_transitions(j, count = True, norm=False, separator = ' > ', ):
  """
 ...
 #Funções para o escopo de base
jorns = pd.DataFrame({'path':['A > B > C','A > A > A', 'B > C'],'ocor':[2,3,1]})
...
def tps_by_channel(df,j,ocurrencies,separator = ' > '):
  """
